...
if ! rsync -ra --progress generatestats "${HOST}:/etc/cron.daily/"; then exit 1; fi
ssh $HOST "cd ${DESTINATION} && chown -R www-data:www-data ${DESTINATION} && chmod -R 755 ${DESTINATION} && systemctl reload nginx && systemctl restart cron"
