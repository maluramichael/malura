...
if ! rsync -ra --progress generatestats "${HOST}":"${DESTINATION}"; then exit 1; fi
ssh $HOST "cd ${DESTINATION} && chown -R www-data:www-data ${DESTINATION} && chmod -R 755 ${DESTINATION} && systemctl reload nginx && systemctl restart cron && ./generatestats"
